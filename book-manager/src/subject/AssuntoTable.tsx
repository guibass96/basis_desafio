...
import { AssuntoEntity, deleteSubject } from '../services/subjectService';
import EditSubjectModal from './modal/editSubjectModal';

type ColumnId = keyof AssuntoEntity | 'actions';
...
export default function ColumnGroupingTable({ data, onSuccess }: ColumnGroupingTableProps) {
  ...
  const [subjectToEdit, setSubjectToEdit] = React.useState<AssuntoEntity | null>(null);
  ...
  const handleEdit = (subject: AssuntoEntity) => {
  ...
  return (
    <Paper sx={{ width: '100%', overflowX: 'auto', borderRadius: 2, boxShadow: 3, p: 2 }}>
      <EditSubjectModal open={open} onClose={handleClose} subjectToEdit={subjectToEdit} onSuccess={handleClose} />
              ...
              .map((row) => (
                <TableRow hover key={row.id}>
                  {columns.map((column) => {
                    ...
                    const value = row[column.id as keyof AssuntoEntity];
                    return (
